...
void OV5640_whoami(){
	...
	id_h = OV5640_get_reg(0x300A);
	id_l = OV5640_get_reg(0x300B);
...
void OV5640_init(){
	...
	for (int i = 0; i < sizeof(cfg_init_) / sizeof(cfg_init_[0]); i++){
		iic_send[0] = cfg_init_[i].addr >> 8;
		iic_send[1] = cfg_init_[i].addr & 0x00FF;
		iic_send[2] = cfg_init_[i].data;
...
void OV5640_set_vid_mode(int vid_mode){
	...
	for (int i = 0; i < cnt; i++){
		iic_send[0] = cfg_[i].addr >> 8;
		iic_send[1] = cfg_[i].addr & 0x00FF;
		iic_send[2] = cfg_[i].data;
...
void OV5640_set_awb(int awb_mode){
